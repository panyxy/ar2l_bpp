...
def make_env(env_id, seed, rank, log_dir, allow_early_resets, args, validate=False):
    def _thunk():
        if env_id.startswith("dm"):
            ...
            env = gym.make(
                ...
                load_test_data=args.load_dataset if not validate else args.validate,
                num_box=args.num_box,
                num_next_box=args.num_next_box,
                num_candidate_action=args.num_candidate_action,
                ...
                env_idx=rank,
                num_load_episodes=args.num_eval_episodes if not validate else args.num_val_episodes,
            )
...
def make_vec_envs(args, log_dir, allow_early_resets, validate=False):
    ...
    envs = [
        make_env(env_name, seed, i, log_dir, allow_early_resets, args, validate=validate)
    ...
    if len(envs) >= 1:
        ...
        #context = 'fork' if sys.platform == 'linux' else 'spawn'
