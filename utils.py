...
def get_args():
    ...
    parser.add_argument('--training-algorithm', type=str, default='ppo',
                        help='Choose one training algorithm: acktr | a2c | ppo')
    ...
    parser.add_argument('--load-mix-model', action='store_true', default=False,
                        help='Load the trained Balance model')
    ...
    parser.add_argument('--mix-model-path', type=str,
                        help='The path to load Balance model')
    ...
    parser.add_argument('--sample-left-bound', type=float, default=None,
                        help='The left bound of the uniform distribution')
    parser.add_argument('--sample-right-bound', type=float, default=None,
                        help='the right bound of the uniform distribution')
    parser.add_argument('--unit-interval', type=float, default=None,
    ...
    parser.add_argument('--validate-interval', type=int, default=200)
    ...
    if args.sample_from_distribution and args.sample_left_bound is None: args.sample_left_bound = 0.1 * min(args.container_size)
    if args.sample_from_distribution and args.sample_right_bound is None: args.sample_right_bound = 0.5 * min(args.container_size)
    if args.sample_from_distribution and args.unit_interval is None: args.unit_interval = 0.1
    return args
...
def load_ppo_policy(model_path, policy_model, strict=True):
    ...
    policy_model.load_state_dict(pretrained_state_dict, strict=strict)
    print('Load trained model', model_path)
    return policy_model


def update_linear_schedule(optimizer, epoch, total_num_epochs, initial_lr, minimum_lr):
